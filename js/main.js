...
var TotalStreams;
var header_title = "";
document.scrollingTitle = false;

// Have to wait for the jQuery library to load
$( document ).ready(function() {

	// Hide the Interface
	HideOrDisplayInterface();

	// Show title

	if(document.scrollingTitle)
		ScrollingTitle(0);
...
// Makes the page title scroll and show the names of streamers.

function ScrollingTitle(x)
{
	if(header_title.length >= 20)
	{
		x++;
		if(header_title.length < x+1)
			x = 0;
		document.title = "Multistream|" + header_title.substr(x,20) + header_title.substr(0,20+x-header_title.length);
	}
	else if(header_title.length >= 2)
		document.title = "Multistream|" + header_title;
	else
...
function NonScrollingTitle()
{
	if(header_title.trim().length > 0)
		document.title = header_title.trim();
	else
		document.title = "Multistream";
}
...
function EnableHeaderButtons()
{
	...
	$(".header_link.help, .help_overlay").click(function(){
		ToggleHelpWindow();
	});
	$(".header_link.reposition").click(function()
	{
		if(vid_alignment == "boxed")
			vid_alignment = "vertical";
		else if(vid_alignment == "vertical")
			vid_alignment = "horizontal";
		else
			vid_alignment = "boxed";

		$(this).text(vid_alignment);
		ResizeStreams()
	});
}

function ToggleHelpWindow(hide){
	if(typeof hide == 'undefined')
		hide = false;
	var help_element = $(".help_overlay")
	if(help_element.is(":visible") || hide)
		help_element.hide();
	else
		help_element.show();
}
...
function UpdateDisplayList()
{
	...
	// For changing the document title
	var online_streams = [];
	...
	$.each(stream_list, function(website, streams)
	{
		$.each(streams, function(index,stream_data)
		{
			...
			{
				var status = "online";
				online_streams.push(stream_display_name);
			}
			else
				var status = "offline";
	...
	header_title = online_streams.join(" ") + " ";
	if(!document.scrollingTitle)
		NonScrollingTitle();
}
